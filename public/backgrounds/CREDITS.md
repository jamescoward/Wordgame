# Background Image Credits

All images sourced from [Unsplash](https://unsplash.com) under the [Unsplash License](https://unsplash.com/license).
Images must be manually downloaded and placed here before these backgrounds become available in-game.

| File | Name in Shop | Photographer | Profile |
|------|-------------|--------------|---------|
| `forest-canopy.jpg` | Forest Canopy | Sebastian Unrau | https://unsplash.com/@sebastian_unrau |
| `night-city.jpg` | Night City | Luca Bravo | https://unsplash.com/@lucabravo |
| `golden-beach.jpg` | Golden Hour Beach | Shana Van Roosbroek | https://unsplash.com/@shanavaro |
| `snowy-mountain.jpg` | Snowy Mountain | Rohit Tandon | https://unsplash.com/@sepoys |
| `cherry-blossom.jpg` | Cherry Blossom | Manuel Cosentino | https://unsplash.com/@manucosen |

## Image Specs
- Format: JPEG
- Target size: ~400KB each (optimised for mobile)
- Dimensions: 1080×1920px (portrait)

## Download Instructions
1. Visit each photographer's Unsplash profile (links above)
2. Choose a suitable photo matching the theme
3. Click "Free download" (no API key needed)
4. Resize/optimise to 1080×1920px
5. Save as the filename shown above in this directory
